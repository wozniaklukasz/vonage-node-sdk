...
import { UrlResponse } from '../lib/interfaces/response/index';
...
describe('Meetings > File uploads', () => {
  ...
  test.each(tests)(`Can upload $logoType logo`, async ({
    awsExpected,
    logoType,
  }) => {
    ...
    scope
      ...
      .put(
        `${BASE_PATH}/meetings/themes/${themeOne.themeId}/finalizeLogos`,
        {
          keys: [fields.key],
        },
      )
  ...
  test('Will throw error when file does not exist', async () => {
    await expect(
      ...
      .rejects
      .toThrow('File not-real does not exist');
  ...
  test('Will throw error when AWS fails', async () => {
    ...
    await expect(
      ...
      .rejects
      .toThrow(`Failed to upload to AWS: Invalid according to Policy: Policy expired`);
  ...
  test('Will throw error when apply fails', async () => {
    ...
    await expect(
      ...
      .rejects
      .toThrow(`Could not attach image to theme: key_not_found`);
  ...
  test('Will throw error when apply fails with no message', async () => {
    ...
    await expect(
      ...
      .rejects
      .toThrow(`Could not attach image to theme: FATAL ERROR`);
